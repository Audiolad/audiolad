...
| 20260715160000 | archive_demo_catalog_practices | data | Historical targets (`slug IN (...) AND published_at < 2026-07-15 16:00:00+00`) are `unpublished`/`archived` or later deleted | Original UPDATE was `slug IN (...) AND status='published'`. Starters republished by `20260715170000` are out of this probe. `20260731180000` remaps leftover archived → unpublished. A later published row with the same slug is not a target. Evidence `superseded_by:20260731180000`. |
