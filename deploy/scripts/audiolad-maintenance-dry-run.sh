#!/bin/bash
# Draft privileged Audiolad maintenance dry-run wrapper.
# Intended install path: /usr/local/sbin/audiolad-maintenance-dry-run
...
# Runs only the installed canonical maintenance script with hardcoded --dry-run.
# Never deletes. Never invokes audiolad-deploy. Never restarts PM2/nginx/Docker.
set -Eeuo pipefail

if [[ $# -ne 0 ]]; then
  printf 'ERROR: audiolad-maintenance-dry-run accepts no arguments.\n' >&2
  exit 1
fi
...
exec /usr/local/lib/audiolad/audiolad-maintenance.sh --dry-run
