#!/usr/bin/env bash
# Process author partner activation email outbox against the active release.
#
...
LOCK_FILE="${LOCK_FILE:-/run/audiolad-author-partner-activation-email-outbox.lock}"
LOG_FILE="${LOG_FILE:-/var/log/audiolad/author-partner-activation-email-outbox.log}"
...
OUTPUT="$(
  timeout --signal=TERM --kill-after=15s "$TIMEOUT_SECONDS" \
    "$NPM_BIN" run run:author-partner-activation-email 2>&1
)"
EXIT_CODE=$?
set -e
