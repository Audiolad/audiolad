...
APP_NAME="${PRODUCT_AUDIO_NORMALIZE_PM2_APP:-audiolad-product-audio-normalize-worker}"
ECOSYSTEM="${ECOSYSTEM:-$DEPLOY_TREE/product-audio-normalize-worker.ecosystem.config.cjs}"
PM2_BIN="${PM2_BIN:-pm2}"
SETTLE_SECONDS="${PRODUCT_AUDIO_NORMALIZE_WORKER_SETTLE_SECONDS:-3}"
...
require_online() {
  ...
  if [[ "$status" != "online" ]]; then
    log "product_audio_normalize_worker_not_online app=$APP_NAME status=$status"
    return 1
  fi
  return 0
}
...
if [[ ! -f "$ECOSYSTEM" ]]; then
  log "product_audio_normalize_worker_ecosystem_missing path=$ECOSYSTEM"
  exit 1
fi
...
case "$STATUS" in
  ...
  stopped|errored|stopping|launching|waiting|one-launch-status)
    log "product_audio_normalize_worker_recover app=$APP_NAME status=$STATUS"
    ...
    ;;
  *)
    log "product_audio_normalize_worker_unexpected_status app=$APP_NAME status=$STATUS"
    if ! "$PM2_BIN" restart "$APP_NAME" --update-env; then
      log "product_audio_normalize_worker_restart_failed app=$APP_NAME"
      exit 1
    fi
...
log "product_audio_normalize_worker_online app=$APP_NAME"
