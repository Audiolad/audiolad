#!/usr/bin/env bash
# Process author product-sold email outbox against the active release.
#
...
LOCK_FILE="${LOCK_FILE:-/run/audiolad-author-sale-email-outbox.lock}"
LOG_FILE="${LOG_FILE:-/var/log/audiolad/author-sale-email-outbox.log}"
...
OUTPUT="$(
  timeout --signal=TERM --kill-after=15s "$TIMEOUT_SECONDS" \
    "$NPM_BIN" run run:author-sale-email-outbox 2>&1
)"
EXIT_CODE=$?
set -e
