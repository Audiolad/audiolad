...
assert.match(view, /<WorkflowVisual \/>/);
assert.match(view, /<AttentionVisual \/>/);
assert.match(view, /<ContextVisual \/>/);
assert.match(view, /<IterationVisual \/>/);
for (const value of [
  ...
  "Речь, действие или пространство",
  "Поддерживает настроение и ритм",
  "Перетягивает внимание резкими переходами или лишними элементами",
  "Мягко, плавно, без лишнего внимания",
  "Ровно, спокойно, без резких переходов",
  "Музыка оставляет место речи и сюжету",
  "Музыка формирует атмосферу помещения",
  "1. СНАЧАЛА ОПРЕДЕЛИТЕ, ДЛЯ ЧЕГО НУЖНА МУЗЫКА",
  "2. ДЛЯ ФОНА ЧАСТО ЛУЧШЕ НАЧИНАТЬ С МУЗЫКИ БЕЗ СЛОВ",
  "3. ОПИШИТЕ НАСТРОЕНИЕ, А НЕ ПРОСТО «КРАСИВУЮ МУЗЫКУ»",
  "4. ЗАДАЙТЕ ТЕМП, ИНСТРУМЕНТЫ И ДИНАМИКУ",
  "5. СОБЕРИТЕ ПРОСТОЙ ТЕКСТОВЫЙ ЗАПРОС",
  "6. СОЗДАВАЙТЕ НЕСКОЛЬКО ВАРИАНТОВ",
  "7. СЛУШАЙТЕ ФОНОВУЮ МУЗЫКУ В ТОМ КОНТЕКСТЕ, ДЛЯ КОТОРОГО ОНА СОЗДАНА",
  "8. ДОРАБОТАЙТЕ ЛУЧШИЙ ВАРИАНТ",
