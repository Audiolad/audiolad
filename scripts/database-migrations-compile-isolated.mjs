...
if (!pending.has("20261007150000")) {
  throw new Error("music delivery promotion migration was not included in the disposable replay");
}
applyFile(
...
if (!pending.has("20261101120000")) {
  throw new Error("Wordstat music query seed migration was not included in the disposable replay");
}
runPsql(databaseUrl, [
  "-c",
  `UPDATE public.seo_queries
   SET query_text = 'Музыка для засыпания взрослых!!!',
       frequency = 777,
...
applyFile(
  "reapply Wordstat music query seed",
  resolve(root, "supabase/migrations/20261101120000_seo_wordstat_music_queries_seed.sql"),
);
applyFile(
