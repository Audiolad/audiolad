...
function testPromotionInlineIntegration() {
  const promotionClient = read("src/components/author-dashboard/AuthorPromotionClient.tsx");
  const pagesClient = read("src/components/author-dashboard/AuthorPromoPagesClient.tsx");

  assert(
    promotionClient.includes("AuthorPromoPagesClient"),
    "promotion page renders promo pages inline",
  );
  assert(
    promotionClient.includes("<AuthorPromoPagesClient selectedAuthor={selectedAuthor} />"),
    "promo pages wired to selected author workspace",
  );
  assert(
    promotionClient.indexOf("<AuthorPromoPagesClient") <
      promotionClient.indexOf("{selectedCampaign ? ("),
    "promo pages block rendered before campaign links section",
  );
  assert(
    !promotionClient.includes("AuthorPromotionTabs"),
    "promotion page does not use tabs",
  );
  assert(
    !promotionClient.includes("AuthorPromotionWorkspace"),
    "promotion page does not use separate workspace shell",
  );
  assert(
    pagesClient.includes('params.set("page", "new")') &&
      !pagesClient.includes('params.set("tab", "pages")'),
    "promo page form opens without tab switch",
  );
  assert(!pagesClient.includes('params.delete("author")'), "author query preserved");
  assert(!pagesClient.includes('params.delete("period")'), "period query preserved");
}
...
function testPublishedLinkVisibility() {
  ...
  assert(
    pagesClient.includes("Скопировать ссылку") &&
...
function testPromotionUi() {
  ...
  assert(promotionClient.includes("AuthorPromoPagesClient"), "promotion client embeds promo pages");
  assert(pagesClient.includes("Промостраницы"), "promo pages section title");
  assert(
    pagesClient.includes("Создавайте посадочные страницы с одной, двумя или тремя практиками."),
    "promo pages section subtitle",
  );
  assert(
    pagesClient.includes('text-[18px] font-semibold">Промостраницы</p>'),
    "empty state title",
  );
...
function testPublicPromoRouteExists() {
  ...
  assert(route.includes("robots:"), "public promo route sets robots metadata");
  assert(route.includes("index: false"), "public promo route is noindex");
}
...
function run() {
  ...
  testLinkHelper();
  testPromotionUi();
  testPromotionInlineIntegration();
