...
for (const text of [
  ...
  "десятки сервисов",
  "какую именно музыку вы хотите получить?",
  "Но это не означает, что Суно лучше для любой задачи.",
  "Продавать",
  "Продавать / использовать в продукте",
...
assert.doesNotMatch(view, />Продавать<[^]*>[^]*>Использовать в продукте</);
const cyrillicWordCount = (view.match(/[А-Яа-яЁё]+/g) ?? []).length;
assert.ok(cyrillicWordCount >= 1_800, `page body must retain full content volume; got ${cyrillicWordCount} Cyrillic words`);
