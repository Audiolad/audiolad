...
setGetMaxPublishedProductForTests(async (author,slug)=>{lookup++; return {ok:true,product:{authorSlug:author,productSlug:slug,title:"T",subtitle:null,description:null,authorName:"A",formatLabel:"Практика",coverUrl:null,priceLabel:"Бесплатно",isFree:true,statsLabel:null,contents:[{title:"Трек",position:1,durationSeconds:65}]}};});
try {
  ...
  assert.equal(r.status,200); const ok=await r.json(); assert.equal(ok.product.contents[0].id,undefined); assert.equal(r.headers.get("cache-control"),"no-store");
