-- Attribution behavioral smoke (PR2 harden). Requires foundation + attribution migrations applied.
DO $$
DECLARE
  v jsonb;
  ...
  v_cnt int;
BEGIN
  INSERT INTO auth.users (id) VALUES (v_user_a), (v_user_b), (v_user_c), (v_user_d), (v_user_e)
  ...
  v := public.author_partner_touch_invite('SERGEY', repeat('1', 64), NULL);
  ...
  IF coalesce(v->>'code','') <> 'SERGEY' THEN
  ...
  IF coalesce(v->>'code','') <> 'MARINA' THEN
    RAISE EXCEPTION 'expired should allow new first-touch: %', v;
  END IF;

  v := public.author_partner_touch_invite('EMPTYX', repeat('4', 64), NULL);
  IF coalesce(v->>'result','') <> 'created' THEN
  ...
  SELECT count(*) INTO v_cnt FROM public.author_partner_attributions WHERE referrer_author_id = v_author_empty;
  IF v_cnt <> 0 THEN
    RAISE EXCEPTION 'attributions should cascade on author delete, left %', v_cnt;
  END IF;

  v := public.author_partner_touch_invite('SERGEY', repeat('5', 64), NULL);
  v := public.author_partner_claim_attribution(repeat('5', 64), v_user_d);
  IF coalesce(v->>'result','') <> 'bound' THEN
    RAISE EXCEPTION 'claim failed: %', v;
  END IF;
  DELETE FROM public.author_referrals WHERE invitee_user_id = v_user_d AND activated_at IS NULL;
  SELECT count(*) INTO v_cnt FROM public.author_partner_attributions WHERE token_hash = repeat('5', 64);
  IF v_cnt <> 0 THEN
