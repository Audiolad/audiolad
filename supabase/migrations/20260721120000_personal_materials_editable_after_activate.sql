-- Allow author edits on active personal materials without rotating access tokens.
...
BEGIN
  ...
  IF v_material.status NOT IN ('draft', 'active') THEN
...
COMMENT ON FUNCTION public.update_personal_material_draft(
  ...
  'audiolad:personal-material-update-draft:v3; draft+active editable; access token unchanged';

COMMENT ON FUNCTION public.clear_personal_material_draft_audio(uuid) IS
  'audiolad:personal-material-clear-draft-audio:v2; draft+active; storage cleanup is server-side';
