...
-- Recreate create_practice_order / claim_free_practice so PDP callers can
-- resolve by practice_id. Slugs stay unique per author, not globally.
-- Extra args are defaulted so existing positional callers keep working.

DROP FUNCTION IF EXISTS public.create_practice_order(text, uuid, uuid, text, text, uuid, bigint, text);
...
BEGIN
  ...
  IF p_practice_id IS NOT NULL THEN
    ...
    IF p_practice_slug IS NOT NULL
       AND btrim(p_practice_slug) <> ''
       AND v_practice.slug IS DISTINCT FROM btrim(p_practice_slug) THEN
      RAISE EXCEPTION 'practice_not_found'
        USING ERRCODE = 'P0002';
    END IF;
  ELSE
...
REVOKE ALL ON FUNCTION public.create_practice_order(text, uuid, uuid, text, text, uuid, bigint, text, uuid)
  FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_practice_order(text, uuid, uuid, text, text, uuid, bigint, text, uuid)
  FROM anon;
GRANT EXECUTE ON FUNCTION public.create_practice_order(text, uuid, uuid, text, text, uuid, bigint, text, uuid)
  TO authenticated;

COMMENT ON FUNCTION public.create_practice_order(text, uuid, uuid, text, text, uuid, bigint, text, uuid) IS
  'audiolad:create-order:practice-id; pending order + server-resolved sale price + expected-amount race check + promotion snapshots; resolve by practice_id when provided';

DROP FUNCTION IF EXISTS public.claim_free_practice(text);

CREATE FUNCTION public.claim_free_practice(
...
REVOKE ALL ON FUNCTION public.claim_free_practice(text, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.claim_free_practice(text, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.claim_free_practice(text, uuid) TO authenticated;

COMMENT ON FUNCTION public.claim_free_practice(text, uuid) IS
  'audiolad:library-claim:v3; grants free_claim for published catalog-listed free zero-price practices; idempotent; resolve by practice_id when provided';
