...
BEGIN
  ...
  IF v_order_kind = 'course_upgrade' THEN
...
COMMENT ON FUNCTION public.fulfill_tochka_payment_transactional IS
  'audiolad:payments-p30-course-upgrade:v1; transactional Tochka APPROVED fulfill + repair; product_purchase uses grant_practice_purchase_access; course_upgrade grants target from order snapshot via grant_practice_access; success is not access_inserted; service_role only; never trusts client amount/user/test flags.';
