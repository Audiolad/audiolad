...
-- ---------------------------------------------------------------------------
-- Phase 1B: Product Gallery slides for practice / course / audiobook.
--
-- This is Product Gallery, not a gallery for every publication class.
-- Eligible classes are enforced in the application helper
-- isProductGalleryEligible (practice / course / audiobook only).
-- Release (Music) and post (AudioPost) must not create or expose slides.
--
-- publication_id = practices.id. No PracticeGallery / CourseGallery tables.
-- No class column on slides. No backfill of practices.
...
COMMENT ON TABLE public.publication_gallery_slides IS
  'Product Gallery extra 1:1 slides for practice/course/audiobook (practices.id). Not a gallery for every publication class. Cover stays on practices.';

COMMENT ON COLUMN public.publication_gallery_slides.publication_id IS
  'Publication id = practices.id. One slides table; eligibility is practice/course/audiobook only.';
