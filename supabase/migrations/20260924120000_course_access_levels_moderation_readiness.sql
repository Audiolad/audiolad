...
-- Copies the latest v5 function from 20260913120000 and adds ONLY
-- course access-level catalog checks. Empty practice_access_levels
-- keeps legacy single-purchase behavior unchanged.
...
COMMENT ON FUNCTION public.assert_practice_moderation_ready(uuid) IS
  'audiolad:internal-moderation-readiness:v6; SECURITY DEFINER helper for trusted lifecycle RPCs only; title plus ready audio required; optional content does not gate moderation; course uses per-lesson semantic content; course access-level catalog is optional and checked only when rows exist';
