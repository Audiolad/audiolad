...
-- Scope listener pending reuse to product_purchase after studio_music_license.
-- Recreate create_practice_order with catalog visibility gate.
-- listed/unlisted: buyable by direct link. selected_users: allowlist/author/admin only.
-- Strangers get practice_not_found (no listing leak).

CREATE OR REPLACE FUNCTION public.create_practice_order(
...
BEGIN
  ...
  WHERE o.user_id = v_user_id
    ...
    AND o.status = 'pending'
    AND coalesce(o.order_kind, 'product_purchase') = 'product_purchase'
...
COMMENT ON FUNCTION public.create_practice_order(text, uuid, uuid, text, text, uuid, bigint, text) IS
  'audiolad:create-order:visibility-v2; pending reuse scoped to product_purchase so Studio licenses do not collide; listing/allowlist + server-resolved sale price + expected-amount race + promotion snapshots';
