...
-- Scope course_upgrade pending reuse to order_kind after studio_music_license.
-- Entitled learners may start a sequential course_upgrade even when the
-- course is unpublished. Public / not-entitled buyers still fail the
-- published + commercial catalog gates. Does not grant L2. Does not
-- bypass payment. Replay of an existing key stays before current+1.
...
COMMENT ON FUNCTION public.create_course_upgrade_order(uuid, uuid, integer) IS
  'audiolad:course-upgrade-order:v4; sequential native upgrade. Entitled learners may upgrade unpublished courses. Replay returns the original snapshot before recomputing current+1. Server derives current+1 and upgrade_price for new keys only. Ignores client amount. Requires active entitlement. Does not use already_owned. EXECUTE: authenticated.';
