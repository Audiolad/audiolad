...
-- Replay an existing course_upgrade idempotency key BEFORE recomputing
-- current+1 from live entitlement. A completed L1→L2 must return the
-- original snapshot even when the buyer is already on L2.
...
BEGIN
  ...
  -- True idempotency: return the original snapshot before reading
  -- the current entitlement. Do not recompute current+1 on replay.
  SELECT o.*
  INTO v_existing
...
COMMENT ON FUNCTION public.create_course_upgrade_order(uuid, uuid, integer) IS
  'audiolad:course-upgrade-order:v2; sequential native upgrade. Replay returns the original snapshot before recomputing current+1. Server derives current+1 and upgrade_price for new keys only. Ignores client amount. Requires active entitlement. Does not use already_owned. EXECUTE: authenticated.';
