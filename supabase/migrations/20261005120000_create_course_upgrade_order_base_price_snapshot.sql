...
-- Fill orders.base_price_minor_snapshot on course_upgrade INSERT.
-- The column is NOT NULL with no default (20260823180000). v4 omitted it,
-- so a fresh INSERT raised 23502 and the checkout route mapped it to
-- HTTP 500 at create_course_upgrade_order. Same body as v4 otherwise.
-- Snapshot equals the charged upgrade amount (v_amount_minor).
...
BEGIN
  ...
  -- True idempotency: return the original snapshot before published /
  -- commercial / entitlement gates. A later unpublish must not block
  -- replay of an already created upgrade.
  SELECT o.*
  INTO v_existing
  ...
  IF FOUND THEN
    IF v_existing.user_id IS DISTINCT FROM v_user_id
       ...
       OR v_existing.order_kind IS DISTINCT FROM 'course_upgrade' THEN
  ...
  v_entitled := FOUND AND v_current IS NOT NULL;

  -- Unpublished / unlisted catalog gates apply only to new buyers.
  -- An already-entitled learner (purchase, access link, admin) may
  -- start the next sequential upgrade without the product being public.
...
COMMENT ON FUNCTION public.create_course_upgrade_order(uuid, uuid, integer) IS
  'audiolad:course-upgrade-order:v5; sequential native upgrade. Entitled learners may upgrade unpublished courses. Replay returns the original snapshot before recomputing current+1. Server derives current+1 and upgrade_price for new keys only. Ignores client amount. Requires active entitlement. Does not use already_owned. INSERT sets base_price_minor_snapshot = upgrade amount. EXECUTE: authenticated.';
