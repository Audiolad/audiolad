...
export function AudiobookProjectWorkspace({ project, chapters: initialChapters, authorId }: { project: AudiobookProject; chapters: AudiobookChapter[]; authorId: string }) {
  const router = useRouter(); const fileInput = useRef<HTMLInputElement>(null); const [chapters, setChapters] = useState(initialChapters); const [selected, setSelected] = useState<string | null>(initialChapters[0]?.id ?? null); const [busy, setBusy] = useState(false); const [recorderLocked, setRecorderLocked] = useState(false); const [confirmingDelete, setConfirmingDelete] = useState(false); const [fragments, setFragments] = useState<AudiobookFragment[]>([]); const [uploadError, setUploadError] = useState<string | null>(null); const [retryingFragment, setRetryingFragment] = useState<AudiobookFragment | null>(null); const [renderJob, setRenderJob] = useState<{ status: "queued" | "processing" | "completed" | "failed"; error_message_safe: string | null } | null>(null);
  ...
  useEffect(() => {
    ...
    let active = true;
    const refresh = () => request(`${renderBase}?authorId=${authorId}`).then(({ job }) => { if (active) setRenderJob(job); }).catch(() => {});
    refresh();
  ...
  async function renderChapter() {
    ...
    setBusy(true); setUploadError(null);
    try { const { job } = await request(renderBase, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ authorId }) }); setRenderJob(job); }
