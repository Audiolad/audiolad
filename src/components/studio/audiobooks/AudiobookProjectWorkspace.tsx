...
export function AudiobookProjectWorkspace({ project, chapters: initialChapters, authorId }: { project: AudiobookProject; chapters: AudiobookChapter[]; authorId: string }) {
  ...
  async function deleteBook() { if (recorderLocked) return; setBusy(true); try { await request(`/api/studio/audiobooks/projects/${project.id}?authorId=${authorId}`, { method: "DELETE" }); router.push("/studio/audiobooks"); } finally { setBusy(false); } }
