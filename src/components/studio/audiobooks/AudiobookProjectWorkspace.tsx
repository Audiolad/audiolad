...
export function AudiobookProjectWorkspace({ project, chapters: initialChapters, authorId }: { project: AudiobookProject; chapters: AudiobookChapter[]; authorId: string }) {
  const router = useRouter(); const fileInput = useRef<HTMLInputElement>(null); const [chapters, setChapters] = useState(initialChapters); const [selected, setSelected] = useState<string | null>(initialChapters[0]?.id ?? null); const [busy, setBusy] = useState(false); const [recorderLocked, setRecorderLocked] = useState(false); const [confirmingDelete, setConfirmingDelete] = useState(false); const [fragments, setFragments] = useState<AudiobookFragment[]>([]); const [uploadError, setUploadError] = useState<string | null>(null); const [retryingFragment, setRetryingFragment] = useState<AudiobookFragment | null>(null); const [renderState, setRenderState] = useState<{ job: { status: "queued" | "processing" | "completed" | "failed"; error_message_safe: string | null } | null; isCurrent: boolean }>({ job: null, isCurrent: false });
  ...
  const renderBase = selected ? `${base}/${selected}/render` : null;
  useEffect(() => {
    if (!renderBase) return;
    let active = true;
    let timer: number | undefined;
    const refresh = () => request(`${renderBase}?authorId=${authorId}`).then((state) => {
      if (!active) return;
      setRenderState(state);
      if (state.job?.status === "queued" || state.job?.status === "processing") timer = window.setTimeout(refresh, 3000);
    }).catch(() => {});
    void refresh();
