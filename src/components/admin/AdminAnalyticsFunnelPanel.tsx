    ...
    <section aria-labelledby="admin-funnel-heading" className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-6">
        {[
          ...
          { label: "Недельные слушатели", value: overview.wal, previous: overview.previousWal, delta: overview.walDelta?.compactLabel ?? "—" },
          { label: "Месячные слушатели", value: overview.mal, previous: overview.previousMal, delta: overview.malDelta?.compactLabel ?? "—" },
        ].map((metric) => (
          <article key={metric.label} className="rounded-[18px] border border-[#d9c9f4] bg-white p-4 shadow-sm">
            ...
            {metric.previous !== undefined ? <p className="mt-1 text-xs text-[#9485b4]">{metric.delta} к пред. окну · {metric.previous.toLocaleString("ru-RU")}</p> : <p className="mt-1 text-xs text-[#7042c5]">люди</p>}
          </article>
        ))}
      </div>
