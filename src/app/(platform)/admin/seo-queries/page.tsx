...
export default async function AdminSeoQueriesPage() {
  ...
  const [{ data: queries }, { data: reservations }] = await Promise.all([
    supabase.from("seo_queries").select("id, query_text, normalized_query, source, frequency, intent, recommended_format, audio_fit, created_at, seo_clusters(name)").order("created_at", { ascending: false }),
  ...
  const rows = (queries ?? []).map((item) => {
    ...
    return {
      ...
      cluster: typeof cluster?.name === "string" ? cluster.name : null, intent: item.intent as string | null,
  ...
  return <section><h2 className="text-[21px] font-semibold">SEO-запросы</h2><p className="mt-2 text-sm text-[#796ba0]">Ручное управление поисковыми запросами и их резервированием авторами.</p><div className="mt-5"><AdminSeoQueriesClient initialRows={rows} /></div></section>;
}
