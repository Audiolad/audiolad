...
import { getAdminPathBundle } from "@/lib/admin/analytics-path-queries";
...
export async function GET(request: Request) {
  ...
  try {
    const bundle = await getAdminPathBundle({
      period: state.pathPeriod,
      includeTest: state.includeTestPayments,
      practiceId: state.pathProduct,
      purchaseSurface: state.pathSurface,
    });
