...
import { getAuthorFinanceLedger } from "@/lib/author-finance/queries";
import {
  parseOffset,
  parsePositiveInt,
...
export async function GET(request: Request) {
  try {
    ...
    const ledger = await getAuthorFinanceLedger({
      authorId,
      from: range.from,
      to: range.to,
    ...
    return NextResponse.json({ ...ledger, period: range.period });
  } catch (error) {
