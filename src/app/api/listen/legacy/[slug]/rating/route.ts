...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    return await handlePracticeRatingGet(
      request,
      resolved.authorSlug,
      resolved.productSlug,
    );
  } catch (error) {
    console.error("practice_rating_legacy_get_error", error);
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    return await handlePracticeRatingPut(
      request,
      resolved.authorSlug,
      resolved.productSlug,
    );
  } catch (error) {
    console.error("practice_rating_legacy_put_error", error);
    return NextResponse.json({ error: "internal_error" }, { status: 500 });
  }
}
