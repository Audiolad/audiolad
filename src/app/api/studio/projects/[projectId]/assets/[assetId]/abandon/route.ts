...
import { abandonStudioDirectUpload } from "@/lib/studio/server/direct-upload";
...
export async function POST(_request: Request, context: RouteContext) {
  try {
    ...
    await abandonStudioDirectUpload(projectId, assetId);
    return new NextResponse(null, {
      status: 204,
      headers: { "Cache-Control": "private, no-store" },
    });
  } catch (error) {
    return studioRouteError(error, "studio_project_asset_abandon_error");
  }
}
