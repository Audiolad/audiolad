...
/**
 * Owner signed PDF URL — ownership already verified via claimed RPC.
 * Does not require guest_access_enabled.
 */
export async function createOwnerPdfSignedUrl(input: {
  materialId: string;
  userId: string;
  ...
  const { data: material, error } = await service
    ...
    .select("id, pdf_path, claimed_by_user_id, status, deleted_at")
    .eq("id", input.materialId)
    .maybeSingle();

  if (error) {
    console.error("personal_material_owner_pdf_lookup_error", error.message);
  ...
  const pdfPath = typeof material.pdf_path === "string" ? material.pdf_path.trim() : "";
  ...
  if (signError || !data?.signedUrl) {
    console.error("personal_material_owner_pdf_signed_url_error", signError?.message);
    throw new PersonalMaterialApiError("internal_error", 500);
  }
