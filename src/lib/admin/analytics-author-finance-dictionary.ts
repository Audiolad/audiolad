...
> = {
  ...
  accrued: {
    ...
    hint: "Сумма начислений по подтверждённым оплатам. Считается целочисленно в копейках, остаток от округления остаётся платформе.",
  },
  reversed: {
    label: "Сторнировано",
    kind: "money",
  ...
  netEntitlement: {
    ...
    hint: "Сколько платформа должна авторам по реестру. Это не остаток к выплате: часть суммы может быть на удержании.",
  },
  platformShare: {
  ...
  held: {
    ...
    hint: "Удержание считается по оплате: available_at = дата подтверждения + hold_days. Сторно попадает в ту же корзину, что и начисление, которое оно отменяет.",
  },
  payable: {
    label: "Доступно к выплате",
    kind: "money",
    formula: "обязательство − на удержании",
