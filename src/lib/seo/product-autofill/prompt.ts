...
function faqQ3ListenOnlineInstruction(
  productTitle: string,
  accessMode: ReturnType<typeof productSeoAccessModeFromIsFree>,
): string {
  const access = faqQ3AccessInstruction(accessMode);
  const answer =
    "В Q3.answer скажи, что эту практику можно слушать онлайн на этой странице; не повторяй точное название продукта в ответе.";
  const noSecondary2 =
    "Не используй дополнительный запрос №2 в Q3 только ради SEO. Не подставляй основной запрос вместо названия продукта.";

  if (productTitle) {
    return [
      "Q3 ВСЕГДА про намерение «слушать онлайн».",
      `Используй фактическое название продукта «${productTitle}», а не дополнительный запрос.`,
      `Допустимы естественные варианты вроде «Где можно послушать «${productTitle}» онлайн?», «Где послушать «${productTitle}» онлайн?» или «Можно ли слушать «${productTitle}» онлайн?».`,
      access,
      answer,
      noSecondary2,
    ].join(" ");
  ...
  const genericExample =
    accessMode === "free"
      ? "«Где можно послушать этот материал бесплатно онлайн?»"
      : "«Где можно послушать этот материал онлайн?»";
  return [
    "Q3 ВСЕГДА про намерение «слушать онлайн».",
    "Заголовок карточки сейчас пустой: не выдумывай заголовок и не подставляй основной запрос вместо него.",
    `Используй нейтральную отсылку: этот материал, эта аудиопрактика, эта практика. Допустимый пример: ${genericExample} Также допустимы естественные варианты вроде «Где можно слушать эту аудиопрактику онлайн?».`,
    access,
    answer,
    noSecondary2,
  ].join(" ");
...
function faqItemsSystemInstruction(
  ...
  return [
    ...
    faqQ3ListenOnlineInstruction(productTitle, accessMode),
...
export function buildProductSeoQualityRepairPrompt(
  ...
  const instructions: string[] = [
    "Черновик уже технически валиден. Исправь только обнаруженные качественные проблемы: недостающее покрытие дополнительных запросов, механический повтор точного основного запроса вне разрешённых мест, недостающее намерение Q3 «слушать онлайн» и лишнее направление дополнительного запроса №2 в Q3.",
    ...
    "Перепиши только запрещённые повторы основного запроса, недостающие слоты дополнительных запросов и Q3, если в нём нет намерения «слушать онлайн» или если Q3 засорен дополнительным запросом №2.",
    "Дополнительные запросы должны выполнять свои задачи прежде всего в назначенных публичных блоках. Не используй их в seoDescription только ради SEO.",
  ...
  if (!coverage.listenOnlineIntent || coverage.q3Secondary2Contaminated) {
    const productTitle = input.request.title.trim();
    const contaminationNote = coverage.q3Secondary2Contaminated
      ? "Убери из Q3 лишнее направление дополнительного запроса №2: оставь его только в Q2."
      : "Не используй дополнительный запрос №2 в Q3 только ради SEO.";
    const accessHint = productTitle
      ...
      : coverage.accessMode === "free"
        ? "Продукт бесплатный: в Q3.question естественно включи «бесплатно», например «Где можно послушать этот материал бесплатно онлайн?»."
        : coverage.accessMode === "paid"
          ? "Продукт платный: не пиши «бесплатно». В Q3.answer скажи, что слушать можно на этой странице после получения доступа."
          : "Доступ не подтверждён: не пиши «бесплатно». В Q3.answer скажи, что слушать можно на этой странице.";
    const titleInstruction = productTitle
      ? `Используй фактическое название продукта «${productTitle}» один раз в вопросе.`
      : "Заголовок карточки пустой: не выдумывай заголовок и не подставляй основной запрос вместо него. Используй нейтральную отсылку (этот материал / эта аудиопрактика).";
    instructions.push(
      `Измени только Q3.question и Q3.answer так, чтобы Q3 закрывал намерение «слушать онлайн». ${titleInstruction} ${accessHint} В ответе скажи, что эту практику можно слушать онлайн на этой странице; не повторяй точное название продукта в ответе. ${contaminationNote} Сохрани Q3.anchor. Не меняй Q2, если он уже закрывает дополнительный запрос №2.`,
    );
  }
