...
function secondaryQuerySystemInstruction(
  ...
  const owned = [
    ...
    "Используй их только как контекст для текста: не добавляй, не удаляй, не изменяй, не переставляй и не возвращай их отдельным полем.",
  ].join(" ");
  ...
  return [
    ...
    "Неловкие поисковые фразы можно склонять в прозе; сохранённые запросы автора остаются байт в байт.",
  ].join(" ");
}
...
export function buildProductSeoSystemPrompt(
  ...
  return [
    ...
    secondaryQuerySystemInstruction(secondaryQueries),
