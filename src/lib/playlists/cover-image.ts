...
export async function processPlaylistCoverImage(
  ...
  if (input.length > PLAYLIST_COVER_MAX_BYTES) {
  ...
  if (!magicMime || !PLAYLIST_COVER_ALLOWED_MIME.has(magicMime)) {
    return { ok: false, code: "invalid_file_type" };
  }

  if (
    normalizedDeclared &&
    PLAYLIST_COVER_ALLOWED_MIME.has(normalizedDeclared) &&
  ...
  try {
    const probe = sharp(input, {
      failOn: "error",
      limitInputPixels: PLAYLIST_COVER_MAX_INPUT_PIXELS,
      sequentialRead: true,
      animated: false,
    });
    ...
    if (
      ...
      meta.width * meta.height > PLAYLIST_COVER_MAX_INPUT_PIXELS
    ) {
    ...
    const { data, info } = await sharp(input, {
      ...
      .rotate()
      .resize(PLAYLIST_COVER_OUTPUT_SIZE, PLAYLIST_COVER_OUTPUT_SIZE, {
        fit: "cover",
        position: "centre",
        withoutEnlargement: false,
      })
      .webp({
        quality: PLAYLIST_COVER_WEBP_QUALITY,
        effort: 4,
      })
    ...
    if (
      !data.length ||
      info.width !== PLAYLIST_COVER_OUTPUT_SIZE ||
      info.height !== PLAYLIST_COVER_OUTPUT_SIZE
    ) {
      return { ok: false, code: "corrupt_image" };
    }
    ...
    // Confirm EXIF/orientation metadata are not carried into output.
    const outMeta = await sharp(data).metadata();
    if (outMeta.orientation || outMeta.exif || outMeta.icc) {
      // Re-encode once more without metadata if anything leaked.
      const cleaned = await sharp(data)
        .webp({ quality: PLAYLIST_COVER_WEBP_QUALITY, effort: 4 })
        .toBuffer({ resolveWithObject: true });
      ...
      return {
        ...
        buffer: cleaned.data,
        contentType: "image/webp",
        width: cleaned.info.width ?? PLAYLIST_COVER_OUTPUT_SIZE,
        height: cleaned.info.height ?? PLAYLIST_COVER_OUTPUT_SIZE,
      };
    }
