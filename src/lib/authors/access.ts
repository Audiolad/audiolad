...
export function getAuthorAccessBannerMessage(
  ...
  switch (status) {
    ...
    case "commercial_onboarding":
      return "Коммерческая заявка одобрена. Заполните данные для выплат и примите условия сотрудничества, чтобы открыть платные продукты.";
    case "commercial_suspended":
...
export function getPaidPricingDisabledReason(
  ...
  if (status === "commercial_onboarding") {
    return "Сначала заполните данные для выплат и примите условия сотрудничества.";
  }
