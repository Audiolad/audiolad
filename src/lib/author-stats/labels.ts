...
export const AUTHOR_STATS_METHOD_NOTES = [
  ...
  "Время прослушивания считается по фактическому времени воспроизведения аудио. Пауза не учитывается, перемотка вперёд не добавляет время, повторно реально прослушанный участок учитывается.",
  "Учёт времени прослушивания ведётся с даты запуска этой метрики.",
  "Если продукт удалён или передан другому автору, его время остаётся в сумме. Среднее тогда не показываем: запуски считаются по текущему владельцу, и делить старую сумму на оставшиеся запуски было бы неверно.",
] as const;
