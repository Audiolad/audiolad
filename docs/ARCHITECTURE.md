...
- Stage 3A: мобильная витрина UI. `PlaylistCard` принимает только `PlaylistListingItem` (обложка 1:1 или placeholder, title/creator/meta «N аудио · X мин», визуальные сердце и Play без handlers). `PlaylistGrid` переиспользует IntersectionObserver-паттерн product catalog, но ходит в `/api/playlists/catalog`. `PlaylistCatalogFilters`, save/play API и mosaic-обложка не созданы. `CatalogProductGridCard` не используется.
