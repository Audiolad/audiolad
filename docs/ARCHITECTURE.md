...
- Stage 1 каталога плейлистов: отдельный listing-поток `class: "playlist"` (не `kind` продукта). Контракт карточки — `PlaylistListingItem` в `src/lib/playlists/listing-contract.ts`. Сохранения — `playlist_saves`, отдельно от `library_saves`. Будущий маршрут витрины `/playlists/catalog`. Текущие `/playlists` и `/playlists/[id]` не менять. Страницы витрины, карточки и фильтров на этом этапе нет.
