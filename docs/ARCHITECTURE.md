...
- Providers разделены: root `BaseProviders` содержит общую analytics/auth/error инфраструктуру, а `PlatformProviders` монтирует `GlobalAudioPlayerProvider`, PWA и retention только для `(platform)` route group. Полноэкранный `/studio` находится в `(studio)` и не монтирует глобальный плеер.
- Будущий аудиоэфир запускается из Studio. Его завершённая запись должна становиться редактируемым Studio-проектом и проходить тот же последующий pipeline монтажа, обработки, сведения и публикации. Будущая модель проектов должна различать источник материала (например, обычная запись или сохранённый эфир); модель БД пока не определена.
