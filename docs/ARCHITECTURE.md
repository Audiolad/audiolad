...
- Каталог Phase 0/1A: карточка читает `cover` + опциональную `gallery`
  (квадратные слайды витрины, не отдельная сущность). Cover остаётся на
  `practices`; extra slides — `publication_gallery_slides.publication_id`
  = `practices.id`. Course/Offer/Grant таблицы не создаются.
...
- Author UI: `AuthorProductForm` + `CoverUploadBlock` / `useCoverUpload`;
  галерея — `AuthorProductGallery` и
  `GET/POST/PATCH .../gallery`, `DELETE .../gallery/[slideId]`.
  Обложки треков: `POST/DELETE .../audio/[audioId]/cover`.
