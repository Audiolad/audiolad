...
- Stage 2: listing-слой `src/lib/playlists/listing.ts` + `GET /api/playlists/catalog` + серверная страница `/playlists/catalog` (временный простой render). В выдаче только `visibility=public` + `published_at` + `listed_at`. Cursor как у продуктового каталога (`listedAtMs:id`). Карточки, сетки, фильтров UI и play/save кнопок нет; их будущие файлы зафиксированы в `src/lib/playlists/catalog-ui-homes.ts`. `/playlists/catalog` публичный, личный `/playlists` остаётся private.
