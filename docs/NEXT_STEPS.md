...
## 0. Плейлисты — после PR5 (приоритет: высокий)

PR1–PR5 на production (`6a692a2`, release `20260716-053853-6a692a2`).

`/p/[slug]` развёрнут (public + published gate, signed cover, mosaic, copy link, SEO). Migration для PR5 не требовалась.

Далее:

1. Play All между продуктами — только отдельным этапом плеера.
2. Сохранение чужого публичного плейлиста — отдельный этап.
3. Публичный каталог подборок — отдельный этап.
4. Drag-and-drop — не ближайший шаг.
