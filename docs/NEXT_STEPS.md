...
## 0. Плейлисты — после PR3.1 (приоритет: высокий)

PR1 на production. PR2 развёрнут. PR3.1 (membership из Аудиотеки) реализован в рабочей копии: review → commit → migration на test/prod → deploy.

Далее:

1. Ревью и commit PR3.1; применить `20260715280000_playlist_membership_rpc.sql` к production по отдельной команде; deploy.
2. PR3.2/PR4 — страница `/playlists/[id]`, порядок ↑↓, listen одного продукта.
