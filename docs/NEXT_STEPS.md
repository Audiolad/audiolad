...
## 0. Плейлисты — следующие PR после схемы (приоритет: высокий)

PR1 (схема + RLS) подготовлен: `20260715270000_create_playlists.sql` (применить к production отдельно после ревью).

Далее:

1. PR2 — CRUD списка плейлистов (UI + API/RPC).
2. PR3 — добавление продуктов из Аудиотеки (private; entitlement на сервере).
3. PR4 — страница `/playlists/[id]`, порядок ↑↓, listen одного продукта.
4. Позже — публикация public + `/p/[slug]`; Play All между продуктами только отдельным этапом плеера.
