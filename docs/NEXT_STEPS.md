...
## 0. Плейлисты — после PR4 / PR5 (приоритет: высокий)

PR1–PR4 на production (`d4b9860`, release `20260716-045024-d4b9860`; previous `20260716-042225-cbd4db1`).

**PR5 `/p/[slug]`** — в рабочей копии (не commit / не production).

Далее:

1. Review → commit PR5 → deploy (migration не требуется, если только app) → public smoke.
2. Позже — Play All между продуктами только отдельным этапом плеера.
