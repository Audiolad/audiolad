...
- Ratings UX (не security): до 30 с preview PUT отвечает 403 `rating_not_eligible`, PDP показывает общее «Не удалось сохранить оценку…». Не менять UI оценки без отдельного задания.
