...
## 0. Плейлисты — следующие PR после CRUD (приоритет: высокий)

PR1 (схема + RLS) закоммичен. PR2 (CRUD `/playlists` + API) в рабочей копии.

Далее:

1. Применить миграцию PR1 к production по отдельному подтверждению.
2. PR3 — добавление продуктов из Аудиотеки.
3. PR4 — страница `/playlists/[id]`, порядок ↑↓, listen одного продукта.
4. Позже — `/p/[slug]`; Play All между продуктами только отдельным этапом плеера.
