...
## 0. Плейлисты — страница содержимого (приоритет: высокий)

PR1–PR3.1 на production (`24616e7`, release `20260715-204408-24616e7`). Membership из Аудиотеки работает.

Далее:

1. PR3.2 — страница `/playlists/[id]` (просмотр items, удаление; без reorder/Play All).
2. Позже — reorder ↑↓; затем `/p/[slug]`; Play All между продуктами только отдельным этапом плеера.
