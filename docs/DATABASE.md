...
Записи с микрофона сначала сохраняются браузером как локальные IndexedDB
черновики, затем в FIFO порядке используют тот же reserve → signed direct
upload → finalize путь с `source_type='recording'`; отдельной таблицы или
