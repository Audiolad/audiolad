...
Существующие строки не ломаются: оба поля NULL. Для обычных платных практик окно читает catalog preview. Для `publication_class='course'` витринный фрагмент выбирается только из аудио уроков Level 1 с явно заданным окном 30–90 с; L2 / `required_access_level > 1` в preview не входят. Автор задаёт одно L1-аудио и границы в кабинете курса (`/api/author/products/[id]/course/storefront-preview`), в том числе у уже опубликованного курса — это витринная метаданных, не замена файлов урока.
